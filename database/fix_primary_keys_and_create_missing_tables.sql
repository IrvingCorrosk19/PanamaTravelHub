-- Script para agregar PRIMARY KEYS faltantes y crear tablas que faltan

-- Agregar PRIMARY KEY a tours si no existe
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conrelid = 'tours'::regclass 
        AND contype = 'p'
    ) THEN
        ALTER TABLE tours ADD CONSTRAINT "PK_tours" PRIMARY KEY (id);
    END IF;
END $$;

-- Agregar PRIMARY KEY a users si no existe
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
...
-- Agregar PRIMARY KEY a bookings si no existe
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
...
-- Agregar PRIMARY KEY a pages si no existe
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        ...
        ALTER TABLE pages ADD CONSTRAINT "PK_pages" PRIMARY KEY (id);
    END IF;
END $$;

-- Agregar PRIMARY KEY a tour_dates si no existe
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conrelid = 'tour_dates'::regclass 
        AND contype = 'p'
    ) THEN
        ALTER TABLE tour_dates ADD CONSTRAINT "PK_tour_dates" PRIMARY KEY (id);
    END IF;
END $$;

-- Ahora crear las tablas faltantes (usando el script anterior pero corregido)
...
-- Crear tabla blog_comments si no existe (corregido para usar mayúsculas en CHECK)
DO $$
BEGIN
